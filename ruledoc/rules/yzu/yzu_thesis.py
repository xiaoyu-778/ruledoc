...
class YZUThesisRule(YZUMixin, FormatRule):
    ...
    def _is_title_paragraph(self, para: "Paragraph", ctx: "ProcessingContext") -> bool:
        ...
        if getattr(ctx, "_title_para_id", None) is not None:
    ...
    def _is_special_title(self, text: str) -> bool:
        ...
        # 特殊处理：关键词段落可能较长，需要单独检测
        if self._is_keywords_paragraph(text_stripped):
            return True
    ...
    def _is_keywords_paragraph(self, text: str) -> bool:
        """检测是否为关键词段落
        ...
        # 检查是否以"关键词"或"keywords"开头，后跟冒号或空格
        patterns = [
            r"^关键词\s*[:：]",
            r"^keywords\s*[:：]",
        ]
        ...
        for pattern in patterns:
            if __import__("re").match(pattern, text_lower):
                return True
    ...
    def _build_ref_field(
        ...
        if self._has_chinese(display_text):
            self._set_east_asian_font(run_text, self.FONT_SETTINGS["chinese_font"])
